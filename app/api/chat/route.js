  ...
  try {
    if (!process.env.GROQ_API_KEY) {
      return Response.json({ 
        message: 'GROQ_API_KEY missing. Get free from console.groq.com',
        error: 'GROQ_API_KEY missing' 
      }, { status: 500 })
    }
    if (!process.env.GEMINI_API_KEY) {
      return Response.json({ 
        message: 'GEMINI_API_KEY missing. Get free from makersuite.google.com',
    ...
    if (image && (lastUserMsg.includes('tattoo') || lastUserMsg.includes('add'))) {
      try {
        ...
        const visionResult = await geminiModel.generateContent([
          { text: 'Describe this hand/wrist: skin tone, position, lighting. Short detail.' },
        ...
        const prompt = `photorealistic hand, ${imageDesc}, professional black ink tattoo saying "${tattooText}" on wrist, natural skin, realistic lighting, 8k`
    ...
    if (lastUserMsg.includes('photo') || lastUserMsg.includes('image') || lastUserMsg.includes('generate') || 
      ...
      return Response.json({ 
        message: `Image ban gayi ✅ Pollinations AI Free`,
